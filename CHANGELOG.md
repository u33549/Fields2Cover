...
- `f2c::hg::CorridorHL::generateHeadlands` overload taking a swath generator and an objective alongside the angles, which asks every border again on the cells an uncut border left joined. A border no corridor is opened on is one the two cells are already a single piece across, and that piece's swaths need not run the way either half ran alone: two tall cells side by side make one wide cell, whose swaths run along the border they used to end on, so it is not cut at all. The depth that comes out is the narrowest worth trying rather than one certainly wide enough, so the generator is then asked to sweep the result; a piece it cuts into more swaths than there are track lines has a throat in it, and the borders reaching that throat are opened back up to the depth taken without an angle. Over 305 real fields the join changes the cells on 176 of 609 robot-field pairs and the sweep check reopens a border on 120.
...
- `f2c::rp::FreeSpaceRoutePlanner` threw away every visibility edge that runs along a wall of the drivable ground. The edge test samples the segment and asked `holds`, a ray cast that answers no exactly on the boundary, while the crossing test beside it already tolerated that same wall -- and a geodesic is made of exactly those wall-running edges, so the route detoured around the corner of the field instead. On a ring of ground the 60 m run along one side of the crop cost 110.75 m. A sample on the border now counts as standing on the ground, to the tolerance the caller passed. The crossing test is untouched, so an edge that properly cuts an exclusion is still rejected, and the tolerance is deliberately not the sample step: it is for exact-boundary arithmetic, not for slack.
- `f2c::rp::FreeSpaceRoutePlanner` joined two pieces of ground that do not touch. The edge test looks at the segment every sample step and asks whether that sample is on the ground, so a gap narrower than the step was never looked at; the crossing test does not catch it either, since an edge over such a gap runs along the walls rather than properly crossing them. Measured: ground in two pieces 0.24 m apart came back as a single piece with six edges straight over the gap, while 0.30 m apart it did not. Each node is now resolved once to the piece it stands on, and an edge between two different pieces is refused. A swath end belongs to no piece -- it stands off the ground by half the strip its own swath covers -- and is left to the other tests, so no swath is cut off.
...
- `f2c::rp::FreeSpaceRoutePlanner` handed every connection to the path planner running through the corner of the ground it turns at. A shortest path is a geodesic and a geodesic hugs whatever it goes around, so the leg arrives at the border of the crop and leaves along it, and a turn of any radius has to cut into the crop to follow that. Each leg that runs along a border is now laid parallel to it and held `turn room (1 - cos(half the corner it turns))` clear of the border its turn cuts towards -- the offset at which an arc of that radius passes the corner instead of crossing it -- and a corner whose legs both reach a swath end, which cannot be moved, is pushed off that border itself. A leg already further out stays where it is, and a connection whose aligned legs would leave the ground is kept as it was -- the two legs that reach a swath end excepted, since such an end stands off the ground by half the strip its own swath covers, so sampling one of those legs would reject every alignment there is.
- `f2c::pp::PathPlanning::planPathForConnection` left the corner after the route's first leg square whenever that leg was shorter than the turn needed to back off along it. A turn is pinned to the route's last point in exactly that case; the first point never got the same treatment, because the start heading holds no sweep and the corner walk stepped past it. It is pinned at both ends now, so a swath end the route leaves by a short leg turns from the end itself.
- `f2c::pp::PathPlanning::planPathForConnection` followed the route's track even where the single turn between its two ends was the only answer that fits. A reversal farther apart than `Robot::getUturnMaxHop` is not offered the direct turn, and a maneuver spanning the whole track reads as a detour and is refused, so such a connection came back with a square corner. Given a free space, the turn between the ends is now taken when it stays on drivable ground and turns no more than the track does.
- `f2c::pp::PathPlanning::planPathForConnection` rounded a jog -- a step aside and back, pinned at one end -- only when one of the six offsets it tries happened to land in the window that fits. Around an island in the headland that window is about a metre wide, narrower than the steps the offsets take, so those corners were left square. The offset where the jog's own two arcs land, measured in the pinned pose's frame, is now offered as well.
